...
const QuestionForm: React.FC = () => {
    ...
    const [relatedOptions, setRelatedOptions] = useState<{ id: number; name: string }[]>([]);
    const [selectedOption, setSelectedOption] = useState("");
    const [selectedCompany, setSelectedCompany] = useState("");
    const [questionText, setQuestionText] = useState("");

    // 初期ロードで会社、言語、カテゴリーを取得
    useEffect(() => {
    ...
    // 関連オプションを取得
    const fetchRelatedOptions = async (type: "language" | "category", id: string) => {
        try {
            console.log(type, id);
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* 言語またはカテゴリーの選択 */}
            <div className="space-y-4">
                <label className="block text-sm font-medium text-gray-700">Select Step</label>
            ...
            {relatedOptions.length > 0 && (
                <div className="space-y-4">
                    <label className="block text-sm font-medium text-gray-700">Related Options</label>
                    <select
                        className="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                        required
                    >
